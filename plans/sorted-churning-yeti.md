...
Нужно реализовать страницу `/dashboard` по спецификации `specs/user-dashboard-my-tasks.md` и дизайну Figma (node 0:64). Страница показывает задачи пользователя в 3 колонках по статусу (Kanban-стиль). Аутентификация не нужна — данные статические. Все shadcn-компоненты уже установлены.
...
Использует: `Card` (с переопределением дефолтных классов), `Badge`, `CalendarIcon` из `lucide-react`.

**Ключевые стили:**
- `Card` → `className="rounded-md p-[17px] gap-1 flex flex-col shadow-[0px_4px_4px_0px_rgba(174,174,174,0.25)] border-gray-200"`  
  (переопределяем `rounded-xl`, `py-6`, `gap-6`)
- `Badge` цвет по категории через `cn()`:
  - `Design` → `className="rounded bg-amber-50 text-amber-300 font-semibold"`
  - `Front-end` → `className="rounded bg-emerald-50 text-emerald-400 font-semibold"`
- Название задачи: `<p className="text-sm text-slate-900 line-clamp-2">`
- Дата: `<CalendarIcon className="size-4 text-slate-400" />` + `<span className="text-xs text-slate-500">`
...
Простой компонент — заголовок колонки + список `TaskCard`.
...
Заменить стаб полностью:

```tsx
import { Plus } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { TaskColumn } from '@/components/dashboard/task-column'
import { MOCK_TASKS, TaskStatus } from '@/lib/mock-tasks'
...
> Фон `bg-slate-50` ставится на `div`, а не на body — layout уже обеспечивает `min-h-screen`.
...
Ожидаемый результат:
- 3 колонки с заголовками To Start / In Progress / Done
- Карточки с цветными бейджами, названием и датой
- Кнопка "Create task" в правом верхнем углу
- Фон страницы светло-серый (`slate-50`)
