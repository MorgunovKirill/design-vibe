...
- Нужен ли sidebar/навигация внутри `(platform)/layout.tsx` или он добавляется позже?
- Какой формат хранения дат предпочтителен для отображения (`toLocaleDateString` vs `date-fns`)?
- Нужна ли возможность перетаскивания карточек между колонками (drag & drop) в этой итерации?
